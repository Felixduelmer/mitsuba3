...
class SpotUltrasoundLight final : public Emitter<Float, Spectrum> {
public:
    MI_IMPORT_BASE(Emitter, m_flags, m_medium, m_to_world)
    MI_IMPORT_TYPES(Scene, Texture)

    SpotUltrasoundLight(const Properties &props) : Base(props) {
    ...
    Float falloff_curve(const Vector3f &d, Mask /*active*/) const {
        ...
        Float center_weight = dr::pow(cos_theta, 2);
        Float beam_res = dr::select(
            cos_theta >= m_cos_beam_width, 1.f,
            (m_cutoff_angle - dr::acos(cos_theta)) * m_inv_transition_width * center_weight);
...
MI_IMPLEMENT_CLASS_VARIANT(SpotUltrasoundLight, Emitter)
MI_EXPORT_PLUGIN(SpotUltrasoundLight, "Spot emitter")
